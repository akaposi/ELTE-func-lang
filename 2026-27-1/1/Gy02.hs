...
mapList f ls = case ls of
  Cons a as -> Cons (f a) (mapList f as)
  Nil -> Nil
...
instance Functor (Either fixed) where
  fmap :: (b -> c) -> Either fixed b -> Either fixed c
  fmap _ (Left fixed) = Left fixed
...
instance Functor (TriEither fixed1 fixed2) where
  ...
  fmap _ (LeftT fixed1) = LeftT fixed1
  fmap _ (MiddleT fixed2) = MiddleT fixed2

instance Functor (BiList fixed) where
  fmap :: (a -> b) -> BiList fixed a -> BiList fixed b
  fmap f (BCons b bs) = BCons (f b) (fmap f bs)
  fmap f (ACons fixed fs) = ACons fixed (fmap f fs)
...
instance (Functor f, Functor g) => Functor (Compose f g) where
  ...
  fmap f (Compose fga) = Compose (fmap (\ga -> fmap f ga) fga)
...
-- instance Functor (UselessF fixed)
-- Gyakorlás:

data Tree a = Leaf | Node (Tree a) a (Tree a) deriving (Eq, Show, Functor)
...
instance Functor (SplitTree fixed) where
  ...
  fmap f (SplitTree ta a b tb) = SplitTree ta a (f b) (fmap f tb)

instance (Functor f) => Functor (Free f) where
  fmap :: Functor f => (a -> b) -> Free f a -> Free f b
  fmap f (Pure a) = Pure $ f a
  fmap f (Free fFfa) = Free (fmap (fmap f) fFfa)
