...
-- evalBExp env (Imp be be2) = not (evalBExp env be) || (evalBExp env be && evalBExp env be2)
...
evalBExp env (Ref s) = fromJust (lookup s env)

parseAndEval :: BEnv -> String -> Bool
parseAndEval env s = evalBExp env $ fromJust (evalParser bExp s)
...
tBracket = char' '[' *> tNode <* char' ']'

tLeaf :: Parser (Tree Int)
tLeaf = Leaf <$> (char' '(' *> number <* char' ')')

tAtom :: Parser (Tree Int)
tAtom = tBracket <|> tLeaf

tNode :: Parser (Tree Int)
-- tNode = liftA2 Node tAtom (char' '+' *> tAtom)
