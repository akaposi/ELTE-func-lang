...
-- egy `Div :: Exp -> Exp -> Exp` konstruktort az eddigi kifejezések mellé. Ez
...
pDoubleLit = do
  neg <- optional (char '-')
  xs <- some (satisfy isDigit)
  char '.'
  ys <- some (satisfy isDigit) <* ws
  let x :: Double
      x = read (xs ++ "." ++ ys)
  case neg of
    Nothing -> pure x
...
data Val  = VInt Int | VBool Bool | VDouble Double
type Env  = [(Name, Val)]
type Eval = State Env
...
data BinOp =
  ...
  | DoubleOrInt (Int -> Int -> Int) (Double -> Double -> Double)

-- Megvalósítás: operátor kiértékelés inputja az típus, és egy adott típusú függvény
--   binOp (Int -> Int -> Int) (+) "+" e1 e2  -- értelmezi a (+)-t
-- (tipp: típusosztállyal talán meg lehet oldani)
...
binOp opName e1 e2 f = do
  ...
  v2 <- evalExp e2
  case (f, v1, v2) of
    (IntOp        f  , VInt n1 , VInt n2 )       -> pure (VInt  (f n1 n2))
    (DoubleOp     f  , VDouble n1 , VDouble n2 ) -> pure (VDouble (f n1 n2))
    (DoubleOrInt  f g, VInt n1 , VInt n2 )       -> pure (VInt  (f n1 n2))
    (BoolOp f,         VBool b1, VBool b2)       -> pure (VBool (f b1 b2))
    _                                            -> error ("type error in " ++ opName ++ " argument")

evalExp :: Exp -> Eval Val
evalExp e = case e of
  Add e1 e2 -> binOp "+"  e1 e2 (DoubleOrInt (+) (+))
  Mul e1 e2 -> binOp "*"  e1 e2 (DoubleOrInt (*) (*))
  And e1 e2 -> binOp "&&" e1 e2 (BoolOp (&&))
  Or  e1 e2 -> binOp "||" e1 e2 (BoolOp (||))
  Div e1 e2 -> binOp "/"  e1 e2 (DoubleOp (/))
  Eq  e1 e2 -> do
    v1 <- evalExp e1
    v2 <- evalExp e2
    case (v1, v2) of
  ...
  --   v2 <- evalExp e2
  --   case (v1, v2) of
  --     (VDouble n, VDouble m) -> pure (VDouble (n / m))
