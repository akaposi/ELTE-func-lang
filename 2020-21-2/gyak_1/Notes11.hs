...
-- jövő héten: vizsgasor (kis feladatok) + while parser/interpreter
--   utolsó BEAD: bármi itteni kisfeladatok témájából
...
instance Functor (D a) where
  fmap f (Con1 n a)     = Con1 n a
  fmap f (Con2 x b)     = Con2 x (f b)
  fmap f (Con3 b mb bs) = Con3 b (fmap f mb) (fmap f bs)

  -- fmap f (Con3 x mb bs) = case mb of
  --   Nothing -> Con3 x Nothing (map f bs)
  --   Just b  -> Con3 x (Just (f b)) (map f bs)

data D' a b = Con3' Bool (Int -> Maybe (Maybe (Int -> [(a, b)])))

-- convertMaybe :: Maybe a -> Maybe b
-- convertMaybe Nothing = Nothing
-- convertMaybe _       = undefined

instance Functor (D' a) where
  fmap f (Con3' x g) = Con3' x $ \n -> case g n of
    Nothing       -> Nothing
    Just Nothing  -> Just Nothing
    Just (Just h) -> Just $ Just $ \m -> map (\(a, b) -> (a, f b)) (h m)

  fmap f (Con3' x g) = Con3' x (fmap (fmap (fmap (fmap (fmap (fmap f))))) g)

  -- fmap f (Con3' x g) = Con3' x ((fmap . fmap . fmap . fmap . fmap . fmap) f g)

  -- fmap f (Con3' x g) =
  --   Con3' x ((fmap `fmap` fmap `fmap` fmap `fmap` fmap `fmap` fmap `fmap` fmap) f g)
...
mostCommon' as =
    snd
  $ maximumBy (\(n, _) (n', _) -> compare n n')
  $ [(n, a) | a <- as, let n = length $ filter (==a) as]
...
mostCommon t = mostCommon' (foldr (:) [] t)
...
instance Foldable RoseTree where
  -- házi feladat: lekövetni a kiértékelés kis fára (akár Debug.Trace függvénnyel)
