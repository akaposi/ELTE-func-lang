...
bRef :: Parser BExp
bRef = Ref <$> some (satisfy isLower) <* ws

bAtom :: Parser BExp
bAtom = bPar <|> bLit <|> bRef

bNot :: Parser BExp
-- bNot = Not <$> (char' '!' *> bAtom) <|> bAtom
bNot = prefixAssoc Not (char' '!') bAtom

bAnd :: Parser BExp
-- bAnd = foldr1 And <$> sepBy1 bNot (char' '^')
bAnd = infixRight And bNot (char' '^')

bOr :: Parser BExp
-- bOr = foldr1 Or <$> sepBy1 bAnd (char' 'v')
bOr = infixRight Or bAnd (char' 'v')

bImp :: Parser BExp
bImp = infixRight Imp bOr (string' "->")

bEq :: Parser BExp
bEq = infixNonAssoc Eq bImp (char' '=')
...
evalBExp :: BEnv -> BExp -> Bool
evalBExp env (Lit b) = b
evalBExp env (Not be) = not (evalBExp env be)
evalBExp env (And be be2) = evalBExp env be && evalBExp env be2
evalBExp env (Or be be2) = evalBExp env be || evalBExp env be2
-- evalBExp env (Imp be be2) = if not (evalBExp env be) then True else evalBExp env be2
evalBExp env (Imp be be2) = not (evalBExp env be) || evalBExp env be2
evalBExp env (Eq be be2) = evalBExp env be == evalBExp env be2
evalBExp env (If be be2 be3) = if evalBExp env be then evalBExp env be2 else evalBExp env be3
evalBExp env (Ref r) = fromJust (lookup r env)

parseEval :: BEnv -> String -> Bool
parseEval env exp = evalBExp env $ fromJust $ evalParser bExp exp
...
tBracket = between (char' '[') tNode (char' ']')

tAtom :: Parser (Tree Int)
tAtom = tBracket <|> tLeaf

tNode :: Parser (Tree Int)
tNode = infixNonAssoc Node tAtom (char' '+')
-- tNode = do
--   char' '['
--   l <- tAtom
--   char' '+'
