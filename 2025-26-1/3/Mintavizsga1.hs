...
instance Foldable Heap where
  ...
  foldMap f (Node h1 a h2) = foldMap f h1 <> f a <> foldMap f h2

instance Functor Heap where
  fmap f Empty = Empty
  fmap f (Node h1 a h2) = Node (fmap f h1) (f a) (fmap f h2)
...
-- verifyInvariant Empty = True
-- verifyInvariant (Node l a r) = foldr min a l == a && foldr min a r == a && verifyInvariant l && verifyInvariant r
verifyInvariant :: Ord a => Heap a -> Bool
verifyInvariant Empty = True
verifyInvariant (Node l a r) = vI a l && vI a r
  where
    vI a Empty = True
    vI a (Node l b r) = b <= a && vI b l && vI b r


heapDepth :: (Ord a, Num a) => Heap b -> a
heapDepth Empty = 0
heapDepth (Node l a r) = 1 + max (heapDepth l) (heapDepth r)

type MonadNav m = (MonadReader [(String, (Int, Int))] m, MonadState [String] m, MonadIO m)

audit :: MonadNav m => String -> m Int
audit s = do
  x <- get
  if s `elem` x
    then do
    y <- ask
    case lookup s y of
      Nothing -> liftIO (putStrLn $ s ++ " adót csal") >> return 0
      Just (a,b) -> return (a - b)
    else return 0
