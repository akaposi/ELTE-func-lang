...
f (x:xs) = do
  b <- readLn :: IO Bool
  sum <- f xs
  if b then return (x + sum) else return (sum - x)
...
gerike = pushS 
  ...
  >>= \l2 -> pushS
  >>= \l3 -> pushS
  >>= \l4 -> return $ length $ filter (== Red) [l1, l2, l3, l4]
...
sumK n = takeK n >>= \ls -> return $ sum ls
...
-- Asszertációs hiba
-- Üres lista hiba
data CustomError = Div0Error | AssertionError | EmptyListError
  deriving (Eq, Show)

-- Példa: biztonságos osztás
safeDiv :: Integral a => a -> a -> Either CustomError a
safeDiv n 0 = Left Div0Error
...
assert True = return ()
assert False = throwError AssertionError

-- Biztonságos osztás
safeDivE :: Integral a => a -> a -> Except CustomError a
safeDivE n 0 = throwError Div0Error
safeDivE n k = return $ div n k
...
safeDivF n k = catchError (safeDivE n k) (\_ -> return 0)
