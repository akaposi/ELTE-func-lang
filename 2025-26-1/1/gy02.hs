...
instance Functor (TriEither fixed1 fixed2) where
  ...
  fmap _ (LeftT e1) = LeftT e1
  fmap _ (MiddleT e1) = MiddleT e1
...
instance Functor (BiList fixed) where
  ...
  fmap f (ACons fixed abs) = ACons fixed (fmap f abs)
  fmap f (BCons a abs) = BCons (f a) (fmap f abs)
...
data VeryCompose f g h a = VeryCompose (f (g (h a))) deriving (Eq, Show)
...
instance (Functor f, Functor g, Functor h) => Functor (VeryCompose f g h) where
  fmap :: (Functor f, Functor g, Functor h) => (a -> b) -> VeryCompose f g h a -> VeryCompose f g h b
  fmap f (VeryCompose fgha) = VeryCompose (fmap (fmap (fmap f)) fgha)
