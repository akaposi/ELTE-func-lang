...
io2 = do
  ...
  let len = length l
  putStrLn "-----------------"
  replicateM_ len (putStrLn l)
  -- printNTimes l len

printNTimes :: String -> Int -> IO ()
printNTimes s 0 = return ()
printNTimes s n = do
  putStrLn s
  printNTimes s (n-1)
...
io3 = do
  l <- getLine
  if elem 'x' l then return () else io3
...
io4 = do
  l <- getLine
  let db = length $ filter isLower l
  -- let db = length $ filter (\c -> elem c ['a'..'z']) l
  -- let db = length $ filter (`elem` ['a'..'z']) l
  print db
  io4
