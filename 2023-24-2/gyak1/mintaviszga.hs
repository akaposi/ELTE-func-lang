...
instance Functor f => Functor (Infinitree f) where
  fmap :: (a -> b) -> Infinitree f a -> Infinitree f b
  fmap g a = case a of
    (Leaf a) -> Leaf $ g a
    (Node h) -> Node $ fmap (fmap g) h

instance Foldable f => Foldable (Infinitree f) where
  foldr :: (a -> b -> b) -> b -> Infinitree f a -> b
  foldr g b a = case a of
        (Leaf a) -> g a b
        (Node h) -> foldr (flip (foldr g)) b h
...
instance (Traversable f) => Traversable (Infinitree f) where
  ...
  sequenceA a = case a of
    (Leaf a) -> Leaf <$> a
    (Node h) -> Node <$> sequenceA (fmap sequenceA h)
...
semiFlattener a = case a of
    (Node a) -> (Left Down) : (concat $ map (\ g -> (semiFlattener g) ++ [Left Up]) a)
    (Leaf a) -> [Right a]

semiUnflattener :: HasCallStack => [Either Dir a] -> Infinitree [] a
semiUnflattener a = f a (Node []) where
    f ((Right a) : xs) (Node []) = f xs (Leaf a)
    f ((Right a) : xs) (Node h) = f xs (Node ((Leaf a) : h))
    f ((Left Down) : xs) (Node []) = let 
        fs = takeWhile (\ e -> case e of Left Up -> False; _ -> True) xs
        sn = drop 1 $ dropWhile (\ e -> case e of Left Up -> False; _ -> True) xs
        re = semiUnflattener fs
         in f sn re
    f ((Left Down) : xs) (Node h) = let 
        fs = takeWhile (\ e -> case e of Left Up -> False; _ -> True) xs
        sn = dropWhile (\ e -> case e of Left Up -> False; _ -> True) xs
        re = semiUnflattener fs
         in f sn (Node (re : h))
    f [] acc = acc
    f b a =  error "This can not happen!"

-- >>> semiUnflattener [Left Down, Left Up]
-- Node []

data ATMError = NotEnoughFunds String | NoUser
    deriving (Eq, Show)

type ATM = WriterT [String] (ReaderT (Maybe String) (StateT [(String, Int)] (Except ATMError))) ()

runAtm atmMonad initialState initialUser = runExcept $ (flip runStateT) initialState $ runReaderT (runWriterT atmMonad) initialUser

loginAs :: String -> ATM -> ATM
loginAs credit m = do
    tell ["Card " ++  credit ++ " is used"]
    local (const (Just credit)) m

withdrawOrDeposit :: Int -> ATM -> ATM
withdrawOrDeposit cred m = do
    user <- ask
    case user of
        Nothing -> lift $ lift $ lift $ throwE NoUser
        Just u -> do
            st <- get
            case lookup u st of
                    (Just v) -> if (v + cred) < 0 then lift $ lift $ lift $ throwE $ NotEnoughFunds "neg funds!" else put ((u, cred +  v) : st)
                    Nothing -> if cred < 0 then lift $ lift $ lift $ throwE $ NotEnoughFunds "Can not create user with neg funds!" else put ((u, cred) : st)
