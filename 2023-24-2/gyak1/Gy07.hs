...
char :: Char -> Parser ()
char c = satisfy (c ==) *> eof

-- Parseoljunk akármilyen karakter
anychar :: Parser Char
anychar = satisfy ((flip elem) $ ['A'..'z'] ++ '*' : ['0'..'9'])
...
-- >>> runParser (asum' [char 'a', char 'b']) "abrakadabra"
-- Nothing
...
-- [A-Z]*[1,2,3,9]?(A|Z)$
p11 :: Parser ()
p11 = many' (satisfy (\x -> x >= min 'A' 'Z' && x <= max 'A' 'Z')) *> optional' (char '1' <|> char '2' <|> char '3' <|> char '9') *> ( char 'A' <|> char 'Z' ) *> eof
