...
instance Foldable Tree where
  ...
  foldr f b (Leaf as) = foldr f b as
  foldr f b (Node l r) = foldr f (foldr f b r) l

  foldMap :: Monoid m => (a -> m) -> Tree a -> m
  foldMap f (Leaf as) = foldMap f as
  foldMap f (Node l r) = foldMap f l <> foldMap f r

instance Traversable Tree where
  traverse :: Applicative f => (a -> f b) -> Tree a -> f (Tree b)
  traverse f (Leaf as) = Leaf <$> traverse f as
...
toLists :: Tree a -> [[a]]
toLists t = go t [] where
  go (Leaf as)  acc = as : acc
...
treeGetLines = traverse (\_ -> getLine)
...
doubleLit :: Parser Exp
doubleLit = do
  signFun <- (negate <$ char '-') <|> pure id
  a <- some (satisfy isDigit)
  char '.'
  b <- some (satisfy isDigit)
  ws
  pure $ DoubleLit $ read (a ++ "." ++ b)
...
divExp :: Parser Exp
divExp = leftAssoc Div notExp (char' '/')

mulExp :: Parser Exp
mulExp = rightAssoc Mul divExp (char' '*')
...
evalExp env e = case e of
  ...
  DoubleLit n -> VDouble n
  Div e1 e2 -> case (evalExp env e1, evalExp env e2) of
    (VDouble n1, VDouble n2) -> VDouble (n1 / n2)
    _                        -> error "type error"
