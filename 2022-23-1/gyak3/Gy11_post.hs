...
pBNot :: Parser BExp
pBNot = prefix BNot pAtom (char' '!')

pBAnd :: Parser BExp
pBAnd = rightAssoc BAnd pBNot (char' '^')

pBOr :: Parser BExp
pBOr = rightAssoc BOr pBAnd (char' 'v')

pBEq :: Parser BExp
pBEq = nonAssoc BEq pBOr (string' "==")
...
evalBExp (BNot e) = not (evalBExp e)
evalBExp (BEq e1 e2) = (evalBExp e1) == (evalBExp e2)
...
data Exp =
  ...
  | StringLit String    -- String literál (csak betűk)
...
atom =
    ...
    <|> (StringLit <$> (char '"' *> many (satisfy isAlpha) <* char' '"'))
    <|> pParensOrPair
    -- "     asdad"
