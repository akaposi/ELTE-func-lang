...
instance Functor MaybeTree where
  ...
  -- fmap f (Node l (Just x) r) = Node (fmap f l) (Just (f x)) (fmap f r)
  fmap f (Node l x r) = Node (fmap f l) (fmap f x) (fmap f r)

instance Foldable MaybeTree where
  foldr f z (Leaf a) = f a z
  foldr f z (Node l x r) = l'
    where
      r' = foldr f z r
      x' = foldr f r' x
      l' = foldr f x' l
...
instance Traversable MaybeTree where
  ...
  traverse f (Node l x r) = Node <$> (traverse f l) <*> (traverse f x) <*> (traverse f r)
...
countJusts (Node l (Just _) r) = countJusts l + countJusts r + 1
...
shiftElems d tr = evalState (traverse go tr) d where
  go :: a -> State a a
  go x = do
    st <- get
    put x
...
-- shiftElems' :: a -> MaybeTree a -> MaybeTree a
-- shiftElems' d (Leaf a) = Leaf d
-- shiftElems' d (Node l x r) = case shiftElems' d l of
--   Node mt m_a mt' -> _
--   Leaf a -> 
