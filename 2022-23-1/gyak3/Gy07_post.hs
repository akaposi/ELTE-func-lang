...
-- traverse id [Right 1, Right 2, Right 3] == Right [1,2,3]
-- id :: a -> a ---- (a -> f b)
-- f :: Either
-- t :: []
-- a :: Int
-- b :: Either Int
-- id :: (a -> a) -- (Right Int -> Right Int)
...
-- traverse :: Applicative f => (a -> f b) -> t a -> f (t b)
maxs' :: [Int] -> [Int]
maxs' ns = evalState (traverse go ns) minBound where -- sequence . (fmap go ns) [1, 3, 5] [go 1, go 3, go 5]
  go :: Int -> State Int Int
  go a = do
    m <- get
    if a > m then put a >> return a else return m
...
replaceLeaves as t = fst . (go t as) where
  go :: Tree a -> [a] -> (Tree a, [a])
  go t [] = (t, [])
  go (Leaf e) (a : as) = (Leaf a, as)
  go (Node l r) (a : as) = (Node l' r', as') where
    l' = undefined -- go -val kéne valamit csinálna (touple visszatérési érték)
    r' = undefined
