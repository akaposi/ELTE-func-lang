...
digitsToDecimalFraction digits = foldr (\curr acc -> acc / 10 + fromIntegral curr) 0 digits / 10
...
-- pMul :: Parser Exp
-- pMul = rightAssoc Mul pPlus (char' '*')
...
evalString str = case evalParser pExp str of
  Nothing -> error "Wrong expression!"
