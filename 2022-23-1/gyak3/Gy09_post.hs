...
-- map digitToInt p0'
posHex :: Parser Int
posHex = hexaToDecimal <$> p0'

hexaToDecimal :: String -> Int
hexaToDecimal hc = foldl (\acc curr -> acc*16 + curr) 0 (map digitToInt hc)
...
--   eof
--   return (length foos)
listLength = length <$> (char' '[' *> sepBy (string' "foo") (char' ',') <* char ']' <* eof)
...
posInt = do
  charList <- some digit
  return (foldl (\acc curr -> acc * 10 + curr) 0 charList)
...
boolList = char' '[' *> sepBy (False <$ string' "False" <|> True <$ string' "True") (char' ',') <* char' ']'

pBool :: Parser Bool
pBool = False <$ string' "False" <|> True <$ string' "True"
...
-- ∙ Followed by a number (ne or more digits)
...
person = do
  ...
  gender <- Boy <$ string "boy" <|> Girl <$string "girl"
  char '.'
  return (MkPerson (fC : rest) age gender)
