...
natural :: Parser Int
natural = foldl (\a r -> a * 10 + r) 0 <$> atLeastOneDigit
--        \r -> sum $ zipWith (*) (reverse r) (map (10^) [0..])
...
sepBy1 a delim = do
  a' <- a
  as <- (delim *> sepBy1 a delim) <|> return []
  return (a' : as)
...
pPow :: Parser Exp
pPow = chainr1 pAtom ((:^) <$ char' '^' <|> (:^^) <$ string' "^^")
  -- rightAssoc (:^) pAtom (char' '^')
