...
-- Let's have an Env type reader environment
-- The function takes a filepath as a parameter. If the user is not an administrator, throw a NotAnAdmin error, and if the file exists, throw a FileExists error
...
-- Essentially, MonadX means that m is a Monad that also has X capabilities
