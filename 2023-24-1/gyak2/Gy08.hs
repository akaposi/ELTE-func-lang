...
---                      V beágyazás
data Gofri f a = Cook a (f (Gofri f a))
  deriving (Functor, Foldable)

instance Traversable f => Traversable (Gofri f) where
  traverse :: (Traversable f, Applicative g) => (a -> g b) -> Gofri f a -> g (Gofri f b)
  traverse f (Cook a g) = Cook <$> f a <*> traverse (traverse f) g
...
aThenB :: Parser ()
aThenB = do--void $ satisfy (== 'a') >> satisfy (== 'b')
  satisfy (== 'a')
  satisfy (== 'b')
  return ()
-- Olvassunk be két adott karaktert egymás után
twoChars :: Char -> Char -> Parser ()
twoChars c1 c2 = void $ satisfy (== c1) >> satisfy (== c2)
