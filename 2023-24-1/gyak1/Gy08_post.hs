...
-- (foo|bar)*kutya?
p0 :: Parser ()
p0 = many_ (string "foo" <|> string "bar") >> optional_ (string "kutya") >> eof
...
p4 :: Parser ()
p4 = optional_ (char 'c') >> some_ (inList ['0'..'9'])


-- ([a..z]|[A..Z])([a..z]|[A..Z]|[0..9])*
p5 :: Parser ()
p5 = (inList ['a'..'z'] <|> inList ['A'..'Z']) >> many_ (inList ['a'..'z'] <|> inList ['A'..'Z'] <|> inList ['0'..'9'])
...
p6 = do
  ...
  char '='
  some_ (inList ['0'..'9'])
  many_ (char ',' >> some_ (inList ['a'..'z']) >> char '=' >> some_ (inList ['0'..'9']))
