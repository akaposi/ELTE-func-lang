...
doubleIfTrue :: (Int -> Maybe Bool) -> Int -> Int
-- doubleIfTrue Nothing x = 0
-- doubleIfTrue (Just False) x = x
-- doubleIfTrue (Just True) x = x * 2
doubleIfTrue f x = case f x of
  Nothing -> 0
  Just False -> x
  Just True -> x * 2
