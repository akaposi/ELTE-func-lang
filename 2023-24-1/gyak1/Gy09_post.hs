...
-- p1' :: Parser ()
-- p1' = some (inList ['0'..'9']) *> char '.' *> optional (string "kis") <* string "ZH"
...
hexaToInt :: String -> Int
hexaToInt = foldl (\acc curr -> acc * 16 + digitToInt curr) 0
...
p1' :: Parser [()]
p1' = ws *> char' '[' *> sepBy1 (string' "foo") (char' ',') <* char' ']'
...
posInt :: Parser Int
posInt = intsToInt <$> (some digit)

intsToInt :: [Int] -> Int
intsToInt = foldl (\acc curr -> acc * 10 + curr) 0

-- Írj egy parsert, ami felsimeri Int-ek vesszővel elválasztott listáit!
-- Példák: "[]", "[    12 , 34555 ]", "[0,1,2,3]"
intList :: Parser [Int]
-- intList = char' '[' *> sepBy (posInt <* ws) (char' ',') <* char' ']'
-- intList = do
--   char' '['
--   list <- sepBy (posInt <* ws) (char' ',')
--   char' ']'
--   return list
...
pList p = char' '[' *> sepBy (p <* ws) (char' ',') <* char' ']'
