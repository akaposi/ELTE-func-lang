...
keywords :: [String]
keywords =
  ["print","not", "true", "false", "while", "if", "do", "end", "then", "else", "fst", "snd"]
...
pStringLit :: Parser Exp
pStringLit = StringLit <$> (char '"' *> many (satisfy isAlpha) <* char' '"')
...
concatExp = rightAssoc Concat subExp (string' "++")

andExp :: Parser Exp
andExp = rightAssoc And concatExp (string' "&&")
...
evalExp env e = case e of
  ...
  Not e -> case evalExp env e of
    VBool b -> VBool (not b)
    _       -> error "type error"
  Var x -> case lookup x (fst env) of
    Just v  -> v
...
statement =
    ...
    <|> (If <$> (keyword' "if" *> pExp <* keyword' "then")
            ...
            <*> (program <* keyword' "end"))
    <|> (Print <$> (keyword' "print" *> pExp <* keyword' "end"))
...
inNewScope ma = do
  ...
  a <- ma
  (env, output) <- get
  put (take l env, output)
  -- modify (take l)
  pure a

updateEnv :: String -> Val -> Env -> Env
updateEnv x v ([], output) = ([(x, v)], output)
updateEnv x v (((y, v'):env), output)
  | x == y    = ((x, v):env, output)
  | otherwise = ((y, v'):fst (updateEnv x v (env, output)), output)
...
evalStatement st = case st of
  ...
  Print e -> do
    (env, output) <- get
    case evalExp (env, output) e of
      VInt n -> put (env, output ++ [show n])
      VBool b -> put (env, output ++ [show b])
      VPair val val' -> put (env, output ++ ['(':show val ++ ',' : show val' ++ ")"])
...
p1 :: String
p1 = "i := 10; acc := 0; while not (i == 0) do acc := acc + i; i := i - 1 end; x := \"alma\"; print 5 end; print (3, 3) end; print \"alma\" end"
