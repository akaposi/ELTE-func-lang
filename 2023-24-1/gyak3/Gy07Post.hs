...
-- (ba|ke)+
p0 :: Parser ()
p0 = void $ some (string "ba" <|> string "ke")

-- crab(,crab)* -- nemüres vesszővel választott crab lista
-- Példa: "crab", "crab,crab"
p1 :: Parser ()
p1 = string "crab" <* many (string ",crab")

-- (ba|ke)*rock
p2 :: Parser ()
p2 = do
	many (string "ba" <|> string "ke")
	string "rock"

-- [a-z]+@hedge\.(com|org|hu)
p3 :: Parser ()
p3 = do
	some $ satisfy (\x -> 'a' <= x && x <= 'z')
	string "@hedge."
...
-- Listás Parser-el működik csak
--p7 = many (satisfy isLower) *> string "win"

-- Mindkét Parser-el működik
p7 = string "win" <|> (satisfy isLower *> p7)
...
integerList = do
	...
	r <- case x of
		Nothing -> return []
		(Just i) -> (i:) <$> many (char ',' *> ws *> integer <* ws)
	ws
	char ']'
	return r
...
-- Helyes példák: "", "()", "()()", "(())()", "(()())", "((()())())"
balanced :: Parser ()
balanced = void $ many (char '(' *> balanced <* char ')')
...
breakAt a = if a < 0 then return [] else do
	b <- readInteger
	bx <- breakAt (a - b)
	return (b:bx)
