...
-- Rákérdezni Kindok voltak-e
-- Rant on Kinds and (* -> *) típusfüggvényekről 
-- vagy polimorfikus típusokról
-- vagy generikus típusokról

-- M : Külömböző módokon lehet pattern matchelni

-- case
mapSingle :: (a -> b) -> Single a -> Single b
mapSingle f s = case s of
  (Single a) -> Single (f a)
...
mapQuintuple f q = 
  let (Quintuple a b c d e) = q in
  Quintuple (f a) (f b) (f c) (f d) (f e)

-- M : either próbáljuk meg let-el
...
-- f doboz, külömböző alakú dobozok vannak
...
-- M : Eddig oldják meg a feladatokat, majd közösen végigmegyünk
...
-- f a doboz alakja, a hogy mi van benne
-- ha akaruzk hogy fmapolhassunk rajta akkor a doboznak 
-- Functornak kell lennie 

-- Példa f = List, a = Int 
-- Lift x : Lift List Int
-- x : (List Int)
