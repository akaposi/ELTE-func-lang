...
data List a = Single a | Empty | Cons a a (List a)
  deriving (Show)

instance Eq a => Eq (List a) where
  Empty         == Empty            = True
  Single a      == Single a'        = a == a'
  Cons a1 a2 as == Cons a1' a2' as' = a1 == a1' && a2 == a2' && as == as'
  _             == _                = False

instance Functor List where
  fmap f Empty           = Empty
  fmap f (Single a)      = Single (f a)
  fmap f (Cons a1 a2 as) = Cons (f a1) (f a2) (fmap f as)

instance Foldable List where
  foldr f b Empty           = b
  foldr f b (Single a)      = f a b
  foldr f b (Cons a1 a2 as) = f a1 (f a2 (foldr f b as))

instance Traversable List where
  traverse f Empty           = pure Empty
  traverse f (Single a)      = Single <$> f a
  traverse f (Cons a1 a2 as) = Cons <$> f a1 <*> f a2 <*> traverse f as
...
find f ta = foldr (\a ma -> if f a then Just a else ma) Nothing ta

findElem :: (a -> Bool) -> Tree a -> Maybe a
findElem = find

-- findElem :: (a -> Bool) -> Tree a -> Maybe a
-- findElem f (Leaf a)    = if f a then Just a else Nothing
-- findElem f (Node1 a t) = if f a then Just a else findElem f t
-- findElem f (Node2 l r) = findElem f l <|> findElem f r
...
--     traverse = undefined -- jobbról-balra!
...
periodicReplace as t = replace (cycle as) t
  -- cycle :: [a] -> [a]
...
-- Tree-t bejárni
-- Maybe Applicative-ban traverse
...
instance Foldable RoseTree where
  foldr f b (Branch a ts) = f a (foldr (\t b -> foldr f b t) b ts)   -- folyamatosan _-t, nézzük a típust

-- Definiáljuk azt a függvényt, amely megszámolja hogy hány elem van egy `RoseTree`-ben!
countElems :: RoseTree a -> Int
countElems = length               -- minden Foldable-re működik
...
genericNumberElems :: Traversable t => t a -> t (a, Int)
genericNumberElems t = evalState (traverse go t) 0 where
  go :: a -> State Int (a, Int)
  go a = do
    n <- get
    put (n + 1)
...
numberElems = genericNumberElems
