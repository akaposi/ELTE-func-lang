...
exampleTree :: MaybeTree Int
exampleTree = (Node (Leaf 5) Nothing (Node (Leaf 1) (Just 8) (Leaf 3)))

-- Írd meg az instance-okat!
instance (Eq a) => Eq (MaybeTree a) where
  (Leaf a) == (Leaf a') = a == a'
  -- (Node l Nothing r) == (Node l' Nothing r') = l == l' && r == r'
  -- (Node l (Just a) r) == (Node l' (Just a') r') = l == l' && a == a' && r == r'
  (Node l ma r) == (Node l' ma' r') = l == l' && ma == ma' && r == r'
  _ == _ = False

instance Functor MaybeTree where
  fmap f (Leaf a)      = Leaf (f a)
  fmap f (Node l ma r) = Node (fmap f l) (fmap f ma) (fmap f r)
  -- fmap f (Node l Nothing r) = Node (fmap f l) Nothing (fmap f r)
  -- fmap f (Node l (Just a) r) = Node (fmap f l) (Just (f a)) (fmap f r)

instance Foldable MaybeTree where
  foldr f z (Leaf a)      = f a z
  foldr f z (Node l ma r) =
    let
      r' = foldr f z r
      ma' = foldr f r' ma
      l' = foldr f ma' l
...
instance Traversable MaybeTree where
  ...
  traverse f (Node l ma r)  = Node <$> (traverse f l) <*> (traverse f ma) <*> (traverse f r)

-- Számold meg a tárolt `Just` konstruktorokat a fában.
countJusts :: MaybeTree a -> Int
countJusts (Leaf a)      = 0
countJusts (Node l Nothing r) = countJusts l + countJusts r
countJusts (Node l (Just _) r) = countJusts l + 1 + countJusts r
