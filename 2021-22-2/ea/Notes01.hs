...
Haskell:
  - "tisztán funkcionális"
    - nem igaz, hogy nincs mellékhatás!

      - Haskell-ben: mutáció, kivételkezelés, konkurrens prog
        IO műveletek

      - Mellékhatás megjelenik a típusokban
         - f :: Int -> Int                 tiszta függvény (tesztelés: input-output)
         - f :: Int -> IO Int              IO mellékhatás lehetséges
         - f :: Int -> Except String Int   String típusú kivételeket dobhat

      - "első osztályú" mellékhatások:
         - (Applicative, Monad osztályok)
         -   pl: nem-determinisztikus programozás:
                   let x = f(x, y, z)      (exotikus más nyelvekben!)
...
-- Eq, Ord, Show osztályok BSc-n

-- motiváció:
--   egyenlőségvizsgálat több típusra
