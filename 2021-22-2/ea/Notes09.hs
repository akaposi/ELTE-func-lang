...
inNewScope ma = do
  env <- get
  let !len = length env   -- BangPatterns-ból: !len, azt jelenti, hogy ezen a pontot szigorúan kiértékeljük
                          -- len-t
  a <- ma
  env' <- get
  put $ take len env'
...
-- vesd össze: teljesen eldobjuk az új állapotot
